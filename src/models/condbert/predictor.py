...
class MaskedTokenPredictorBERT:
    ...
    def generate(self, bpe_tokens_batch, mask_pos_batch, mask_token=True,
        ...
        for num in n_tokens:
            if num == 1:
                continue
            pred_tokens, pred_scores = self.predict_sequence(bpe_tokens_batch, mask_pos_batch, mask_token=mask_token, 
    ...
    def predict_sequence(self, bpe_tokens, mask_pos, n_top, n_units, 
        ...
        for num in range(seq_len):
            seq_tokens, seq_scores = [[0. for _ in range(beam_size)] for _ in range(batch_size)], \
